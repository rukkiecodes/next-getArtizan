  ...
  actions: {
    async signupUser() {
      if (this.name === '' || this.email === '' || this.phone == '' || this.gender == '' || this.stateOfResidence == '' || this.LGA == '' || this.specialisation == '' || this.password === '') {
        ...
        await axios.post(process.env.NODE_ENV == 'production' ? 'https://web-production-563e.up.railway.app/auth/signup' : '/api/auth/signup', {
          name: this.name,
          email: this.email,
          phone: this.phone,
          gender: this.gender,
          ...
          createUserWithEmailAndPassword(auth, this.email, this.password)
            .then(async user => {
              ...
              await setDoc(doc(db, 'artizans', user.user.uid), {
                name: this.name,
                email: this.email,
                phone: this.phone,
                gender: this.gender,
